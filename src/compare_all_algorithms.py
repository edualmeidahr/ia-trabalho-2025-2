"""
Script para comparar GA, ACO e CLONALG no problema de montagem de provas.
...
from src.part3_ga.run_ga import ExamProblem as ExamProblemGA
...
# Parâmetros do problema (mesmos para todos)
TAMANHO_PROVA = 10
ALVO_TEMPO_MIN = 50
ALVO_TEMPO_MAX = 60
...
def run_ga(materia='Física', topico=None, n_iterations=50):
    """Executa GA e retorna resultados."""
    print("\n" + "="*60)
    print("EXECUTANDO ALGORITMO GENÉTICO")
    print("="*60)
    
    banco = BancoDeQuestoes()
    problem = ExamProblemGA(materia, topico, banco)
    
    ga = GA(
        pop_size=GA_PARAMS['pop'],
        fitness_fn=problem.fitness,
        create_ind=problem.create_ind,
        mutate_fn=problem.mutate,
        crossover_fn=problem.crossover,
        cx_rate=GA_PARAMS['cx_rate'],
        mut_rate=GA_PARAMS['mut_rate'],
        elitism=True
    )
    
    start_time = time.time()
    best_solution = ga.run(n_generations=n_iterations, verbose=False)
    ...
    return {
        ...
        'best_fitness': max(ga.history) if ga.history else score,
        'history': ga.history,
        'score': score,
...
def plot_comparison(ga_results, aco_results, clonalg_results):
    """Gera gráficos comparativos entre os três algoritmos."""
    print("\n" + "="*60)
    ...
    iterations_ga = range(len(ga_results['history']))
    iterations_aco = range(len(aco_results['history']))
    iterations_clonalg = range(len(clonalg_results['history']))
    
    ax1.plot(iterations_ga, ga_results['history'], 
             label='Algoritmo Genético', linewidth=2, color='#e74c3c', alpha=0.8)
    ...
    ax1.set_xlabel('Iteração/Geração', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Melhor Fitness', fontsize=12, fontweight='bold')
    ax1.set_title('Convergência: Evolução do Melhor Fitness', 
    ...
    algorithms = ['GA', 'ACO', 'CLONALG']
    fitness_values = [ga_results['best_fitness'], 
                      aco_results['best_fitness'], 
                      clonalg_results['best_fitness']]
    ...
    time_values = [ga_results['execution_time'], 
                   aco_results['execution_time'], 
                   clonalg_results['execution_time']]
    ...
    desvio_tempo_ga = abs(ga_results['tempo_total'] - tempo_medio)
    desvio_tempo_aco = abs(aco_results['tempo_total'] - tempo_medio)
    desvio_tempo_clonalg = abs(clonalg_results['tempo_total'] - tempo_medio)
    
    desvio_dif_ga = abs(ga_results['dificuldade_media'] - ALVO_DIFICULDADE)
    ...
    bars1 = ax4.bar(x - width/2, [desvio_tempo_ga, desvio_tempo_aco, desvio_tempo_clonalg], 
                    width, label='Desvio Tempo (min)', color='#3498db', alpha=0.8)
    bars2 = ax4.bar(x + width/2, [desvio_dif_ga, desvio_dif_aco, desvio_dif_clonalg], 
    ...
    fig.suptitle('Comparação: GA vs ACO vs CLONALG - Problema de Montagem de Provas', 
                 fontsize=16, fontweight='bold', y=0.98)
    
    os.makedirs(REPORTS_PATH, exist_ok=True)
    output_path = os.path.join(REPORTS_PATH, 'ga_aco_clonalg_comparison.png')
    ...
    print("="*60)
    print(f"{'Métrica':<25} {'GA':<15} {'ACO':<15} {'CLONALG':<15}")
    print("-" * 70)
    print(f"{'Fitness Final':<25} {ga_results['best_fitness']:<15.2f} {aco_results['best_fitness']:<15.2f} {clonalg_results['best_fitness']:<15.2f}")
    print(f"{'Tempo Total (min)':<25} {ga_results['tempo_total']:<15.2f} {aco_results['tempo_total']:<15.2f} {clonalg_results['tempo_total']:<15.2f}")
    print(f"{'Dificuldade Média':<25} {ga_results['dificuldade_media']:<15.2f} {aco_results['dificuldade_media']:<15.2f} {clonalg_results['dificuldade_media']:<15.2f}")
    print(f"{'Tempo Execução (s)':<25} {ga_results['execution_time']:<15.2f} {aco_results['execution_time']:<15.2f} {clonalg_results['execution_time']:<15.2f}")
    print(f"\n{'Desvio Tempo (min)':<25} {desvio_tempo_ga:<15.2f} {desvio_tempo_aco:<15.2f} {desvio_tempo_clonalg:<15.2f}")
    print(f"{'Desvio Dificuldade':<25} {desvio_dif_ga:<15.2f} {desvio_dif_aco:<15.2f} {desvio_dif_clonalg:<15.2f}")
    print("="*60)
    
    melhor_fitness = max(ga_results['best_fitness'], aco_results['best_fitness'], clonalg_results['best_fitness'])
    mais_rapido = min(ga_results['execution_time'], aco_results['execution_time'], clonalg_results['execution_time'])
    
    melhor_algo = 'GA' if ga_results['best_fitness'] == melhor_fitness else ('ACO' if aco_results['best_fitness'] == melhor_fitness else 'CLONALG')
    rapido_algo = 'GA' if ga_results['execution_time'] == mais_rapido else ('ACO' if aco_results['execution_time'] == mais_rapido else 'CLONALG')
    
    print(f"\n📊 Análise:")
    print(f"   • Melhor Fitness: {melhor_algo} ({melhor_fitness:.2f})")
    print(f"   • Mais Rápido: {rapido_algo} ({mais_rapido:.2f}s)")
    print("="*60)


def main():
    """Função principal: executa todos os algoritmos e gera comparação."""
    materia = 'Física'
    topico = 'Cinemática'
    
    print("="*60)
    print("COMPARAÇÃO: GA vs ACO vs CLONALG")
    print("="*60)
    ...
    ga_results = run_ga(materia=materia, topico=topico, n_iterations=N_ITERATIONS)
    ...
    # Gera gráficos
    plot_comparison(ga_results, aco_results, clonalg_results)
